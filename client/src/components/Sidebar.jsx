...
const Sidebar = ({ selectedUser, setSelectedUser }) => {
    ...
    return (
        <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl  h-screen overflow-y-scroll text-white ${selectedUser ? "max-md:hidden" : ""}`}>
            ...
            <div className='flex flex-col '>
                {userDummyData.map((user, index) => (
                    <div onClick={()=>{setSelectedUser(user)}} className={`flex items-center gap-2 relative py-2 p-2 rounded-md  hover:bg-[#282142] pl-2 max-sm:text-sm ${selectedUser?._id === user._id && 'bg-[#282142]/50'}`} key={index}>
                        <img src={user?.profilePic || assets.avatar_icon} alt="userpic" className='w-[35px] aspect-[1/1] rounded-full' />
                        <div className=''>
                            <p>{user.fullName}</p>
                            {
                                index < 3 ?
