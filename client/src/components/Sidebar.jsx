...
const Sidebar = ({ selectedUser, setSelectedUser }) => {
    ...
    return (
        <div className={`bg-[#8185B2]/10 h-screen p-5 rounded-r-xl overflow-y-scroll text-white ${selectedUser ? "max-md:hidden" : ""}`}>
            <div className='pb-5'>
                <div className="flex justify-between items-center">
                    ...
                    <div className='relative py-2 group'>
                        ...
                        <div className="absolute top-full right-0 z-20 w-32 p-5 rounded-md
                            bg-[#282142] border border-gray-600 text-gray-100 hidden group-hover:flex flex-col">
            ...
            <div className='flex flex-col'>
                {userDummyData.map((user, index) => (
                    <div
                        ...
                        className={`flex items-center gap-2 relative py-2 p-2 rounded-md hover:bg-[#282142] pl-2 max-sm:text-sm cursor-pointer ${selectedUser?._id === user._id && 'bg-[#282142]/50'}`}
                        key={index}
                    >
                        <img src={user?.profilePic || assets.avatar_icon} alt="userpic" className='w-[35px] h-[35px] rounded-full object-cover' />
                        <div>
                            <p>{user.fullName}</p>
                            {
                                index < 3 ?
